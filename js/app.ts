...
export async function loadWordLists(): Promise<WordLists> {
  try {
    ...
    const [adjs, nouns, diceware] = await Promise.all([
      adjsRes.json() as Promise<string[]>,
      nounsRes.json() as Promise<string[]>,
      dicewareRes.json() as Promise<string[]>
    ]);
 ...
 */
export function handleGeneratePassword(
  elements: AppElements,
  wordLists: WordLists,
  updateCrackTimeFromSettings: () => void
): void {
  ...
  let result: { value: string; entropy: number; crackTime: Record<string, number> } | null = null;
  ...
  if (mode === "easyWrite") {
    ...
    const addDigit = elements.easySayAddDigit?.checked ?? false;
    ...
    const capitalize = elements.passphraseCapitalize?.checked ?? false;
    const addDigits = elements.passphraseAddDigits?.checked ?? false;
    ...
    const charset = getCharset({
      useLower: elements.lowercaseCheckbox?.checked ?? false,
      useUpper: elements.uppercaseCheckbox?.checked ?? false,
      useDigits: elements.digitsCheckbox?.checked ?? false,
      useSymbols: elements.symbolsCheckbox?.checked ?? false,
      symbols
    });
 ...
 */
export function handleGenerateUserIds(elements: AppElements, wordLists: WordLists): void {
    ...
    for (let j = 0; j < 50; j++) {
      ...
      if (mode === "cvc") {
        id = generateCvcId({
          ...
          addDigits: elements.uidAddDigits?.checked ?? false,
          digitsCount: Number(elements.uidDigitsCount?.value),
          addSuffix: elements.uidAddSuffix?.checked ?? false,
        ...
        id = generateWordsId({
          ...
          addDigits: elements.uidWordsAddDigits?.checked ?? false,
 ...
 */
export function updateCrackTimeFromSettings(elements: AppElements, wordLists: WordLists): void {
  ...
  if (mode === "easyWrite") {
    ...
    entropyBits = calculateEntropyBits(length, charset.pool?.length ?? 0);
  }
